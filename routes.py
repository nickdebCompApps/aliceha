...
def register():
    ...
    if form.validate_on_submit():
        print(form.select)
        user = Users(name = form.name.data, username=form.username.data.lower(), email=form.email.data.lower(), number=form.number.data, verified="False", plan='..', numberverification = numbercode(6), token = 'NA', role=form.select.data)
